...
// const {JSDOM} = jsdom
//
// let index_file = fs.readFileSync('./index.html')
//
// const dom = new JSDOM(index_file)
//
...
DLX.Launch = function() {
    ...
    DLX.error = function(m,l) {
        console.error("[ERROR]", m, `(Line ${l})`);
        return DLX.returnCodes.ERROR;
    };

    DLX.warning = function(m,l) {
        console.warn("[WARNING]", m, `(Line ${l})`);
            ...
            while (!DLX.HALTED) {
                ...
                if (DLX.Settings.ALLOWED_EXECS > 0) {
                    ...
                    if (_PROTECT == DLX.Settings.ALLOWED_EXECS) {
                        var goon = true //window.confirm('The program has been running some time. Keep running?');
                        // DLX.RefreshCyclecount();
                        if (goon) _PROTECT = 0;
                        else break;
                    }
    ...
    DLX.ReadProgram = function(p) {
        // console.log('Reading Program', p)
        DLX.program = [];
        DLX.marker = {};
        DLX.PC = 0;
        DLX.HALTED = false;
    ...
    DLX.CheckProgram = function() {
        let ret = DLX.returnCodes.SUCCESS;
        for (let i in DLX.program) {
            let l = DLX.program[i];
            if (l) {
    ...
    DLX.readAddress = function(p, d) {
        ...
        if (/#/.test(p)) {
            ...
            reg = DLX.readRegister(reg);
            address = disp+reg;
        }
        let sel = (address - DLX.Settings.FIRST_ADDRESS)/4 | 0;

        if (sel >= DLX.Memory.length) {
            throw DLX.outOfBounds;
        }

        ret = DLX.Memory[sel];
        if (ret === undefined) {
            ret = DLX.Memory[sel] = 0;
        }
        return [address, parseInt(ret)];
    };

    DLX.writeAddress = function(p, d) {
        let address,ret;
        if (/#/.test(p)) {
            address = DLX.getImmediate(p);
    ...
    DLX.readRegister = function(i, d) {
        ...
        if (DLX.Registers[i] === undefined) {
            DLX.Registers[i] = 0;
        }
        return  d && i == 0 ? DLX.ignoreWrite : DLX.Registers[i];
    };

    DLX.writeRegister = function(i, d) {
        if (!(typeof i == 'number')) {
            i = parseInt(i.slice(1));
        }
        if (0 >= i || 31 < i) {
        ...
        // let ret = d && i == 0 ? DLX.ignoreWrite : DLX.Registers[i];
        // return d ? undefined.aaa : ret;
    };
    ...
    DLX.ExecLine = function(n) {
        ...
        let c = l[0];
        let p = l.slice(1);
        let [r, o] = DLX.commands[c];
        // let o = DLX.commands[c][1];
        let dest,ev;
        if (r === 'ra' || r === 'ar') {
            ...
            // dest = DLX.readRegister(p[0], true);
            ev = DLX.evalParams(p.slice(1),r.slice(1),n);
            if (typeof ev == "number" && ev == DLX.returnCodes.ERROR) {
                ret = ev;
            } else {
                ...
                // }
                DLX.writeRegister(p[0], o(ev[0],ev[1]))
            }
        } else {
            // HALT or TRAP
            DLX.HALTED = true;
    ...
    DLX.commands = {
        ...
        JALR: ['r', true],
        JR:   ['r', false],
        LW:   ['ra', DLX.writeRegister],
        SW:   ['ar', DLX.writeAddress],
        HALT: ['', null],
        TRAP: ['i', null],
        MULT: ['rrr', ops['*']]
    };
...
======================================================================*/
    ...
    DLX.BuildInterface = function() {
        // console.log('Enter BuildInterface')
        ...
        DLX.Registers = new Array(32);
        let i;
        for (i in DLX.Registers) {
            // console.log("Setting Register", i, 0)
            DLX.Registers[i] = 0;
        }
        // generate memory
        // insertInnerHTML = '';
